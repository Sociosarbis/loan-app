...
function FileListPage() {
  ...
  const [folderResource] = createResource(
    async () => {
      try {
    ...
    {
      onHydrated(k, info) {
        fileStore.set("folderId", info.value);
      },
    },
  );
  ...
  const [pageResource] = createResource(
    () =>
      [
        !folderResource.loading ? folderResource() : undefined,
        currentPage(),
      ] as const,
    ([folder_id, page]) => {
      if (!folder_id) {
      ...
      fileStore.set("folderId", folder_id);
      if (!pageLinks().length) {
        setPageLinks([
          `https://graph.microsoft.com/v1.0/me/drive/items/${fileStore.state.folderId}/children?$top=${PAGE_SIZE}`,
        ]);
      }
    ...
    },
    {
      onHydrated(k, info) {
        if (info.value) {
          fileStore.set("files", info.value.files);
          setPageLinks(info.value.pageLinks);
        }
      },
    },
  );
  ...
  const loading = createMemo(
    () => pageResource.loading || folderResource.loading,
  );
